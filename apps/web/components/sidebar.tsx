...
export function Sidebar({ role }: { role?: string }) {
  ...
  return (
    <aside className="w-56 bg-card border-r border-border min-h-screen flex flex-col py-5 px-3 shrink-0">
      <div className="px-3 mb-7 flex items-center gap-2.5">
        ...
        <span className="font-semibold text-sm text-foreground tracking-tight">Blastify</span>
      </div>
      ...
      {role === 'superadmin' && (
        <div className="mt-4 px-3 py-2 rounded-lg bg-accent/10 border border-accent/20">
          <p className="text-[11px] text-accent font-semibold uppercase tracking-widest">Superadmin</p>
        </div>
      )}
    </aside>
  )
